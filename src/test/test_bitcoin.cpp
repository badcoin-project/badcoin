...
{
    const CChainParams& chainparams = Params();
    std::unique_ptr<CBlockTemplate> pblocktemplate = BlockAssembler(chainparams).CreateNewBlock(scriptPubKey, ALGO_SHA256D);
    ...
    while (!CheckProofOfWork(block.GetPoWHash(block.GetAlgo(),Params().GetConsensus()), block.GetAlgo(), block.nBits, Params().GetConsensus())) ++block.nNonce;

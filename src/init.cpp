...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-datacarriersize", strprintf(_("Maximum size of data-carrier (OP_RETURN) scriptPubKey we relay and mine, in bytes including opcodes (default: %u; a 512-byte payload needs 516)"), MAX_OP_RETURN_RELAY));
    ...
    strUsage += HelpMessageOpt("-algo=<algo>", _("Mining algorithm: sha256d, scrypt, groestl, skein, yescrypt"));
    if (showDebug)
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if badcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if badcoin is started while in a temporary directory.\n",
